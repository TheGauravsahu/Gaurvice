    ...
    <div className="flex flex-col mt-4 gap-4">
      {bookings.length > 0 ? (
        bookings.map((item: any, index: any) => (
          <div
            key={index}
            className="shadow-sm border w-full lg:h-[170px] mx-auto rounded-lg overflow-hidden flex flex-col md:flex-row items=center  gap-4"
          >
            <div>
              <Image
                ...
                width={200}
                height={400}
                className="rounded-lg lg:w-fit w-full object-cover"
              />
            </div>
